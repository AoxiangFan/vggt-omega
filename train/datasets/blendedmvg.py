...
from utils.generic_utils import read_image_file, readlines, read_pfm_file
...
class BlendedMVGDataset(GenericMVSDataset):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            scans=scans,
            vggt_frame_sampling=vggt_frame_sampling,
            vggt_valid_range=vggt_valid_range,
            vggt_scan_sample_size=vggt_scan_sample_size,
            single_output_format=single_output_format,
        )
    ...
    def get_all_frame_ids(self, scan_id):
        ...
        files = list(Path(os.path.join(self.scenes_path, scan_id, "cams")).rglob("*_cam.txt"))
        prefixes = [f.stem.replace("_cam", "") for f in files]

        return prefixes
    
    def get_all_poses(self, scan_id):

        frame_ids = self.get_all_frame_ids(scan_id)
        c2w_list = []
        for frame_id in frame_ids:
            c2w, w2c = self.load_pose(scan_id, frame_id)
            c2w_list.append(c2w)
        c2w_list = np.stack(c2w_list, axis=0)
        return c2w_list, frame_ids
