...
from utils.generic_utils import read_image_file, readlines
...
class ScannetDataset(GenericMVSDataset):
    ...
    def get_all_frame_ids(self, scan_id):
        ...
        frame_ids = []
        for path in Path(sensor_data_dir).glob("frame-*.pose.txt"):
            frame_id = path.name[len("frame-"):-len(".pose.txt")]
