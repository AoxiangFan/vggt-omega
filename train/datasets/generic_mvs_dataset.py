...
from utils.geometry_utils import pose_distance, rotz

from .frame_sampling import compute_ranking
...
class GenericMVSDataset(Dataset):
    ...
    def __init__(
        ...
        load_src_depth=True,
        scans=None,
        vggt_frame_sampling=False,
        vggt_valid_range=50,
        vggt_scan_sample_size=100,
        single_output_format=True,
    ):
        """
        Args:
        ...
        self.vggt_frame_sampling = vggt_frame_sampling
        self.single_output_format = single_output_format
        self.num_images_in_tuple = num_images_in_tuple
        self.shuffle_tuple = shuffle_tuple
        
        if self.vggt_frame_sampling:
            self.scans = scans
            self.vggt_valid_range = vggt_valid_range
            self.vggt_scan_sample_size = vggt_scan_sample_size
            self.c2ws_dict = {}
            self.frame_sample_dict = {}
            self.c2w_rankings_dict = {}
            self.frame_ids_list = {}
            self.index_list = list(range(len(scans) * self.vggt_scan_sample_size))
            random.shuffle(self.index_list)
            self.frame_number_matrixcity = {}
        else:
    ...
    def get_all_frame_ids(self, scan_id):

        raise NotImplementedError()
    
    def get_all_poses(self, scan_id):
    ...
    def __getitem__(self, idx):
        ...
        if self.vggt_frame_sampling:
            ...
            scan_index = index // self.vggt_scan_sample_size
            scan_id = self.scans[scan_index]
            frame_sample_index = index % self.vggt_scan_sample_size 

            if "MatrixCity" not in self.dataset_path:
                if scan_id not in self.c2ws_dict:
                    c2ws, frame_ids = self.get_all_poses(scan_id)
                    ranking, dists = compute_ranking(c2ws)
                    self.c2ws_dict[scan_id] = c2ws
                    self.frame_sample_dict[scan_id] = sample_indices(c2ws.shape[0], self.vggt_scan_sample_size)
                    self.c2w_rankings_dict[scan_id] = ranking
                    self.frame_ids_list[scan_id] = frame_ids
                frame_index = self.frame_sample_dict[scan_id][frame_sample_index]
                ranking_for_frame = self.c2w_rankings_dict[scan_id][frame_index]
                frame_tuple_index = sample_with_first(ranking_for_frame[:self.vggt_valid_range], self.num_images_in_tuple)
                frame_ids = [self.frame_ids_list[scan_id][i] for i in frame_tuple_index]
            else:
                scan_id = scan_id.split(":")[0]
                if scan_id not in self.frame_number_matrixcity:
                    folder = Path(self.dataset_path) / scan_id
                    frame_number = len(list(folder.glob("*.png")))
                    self.frame_number_matrixcity[scan_id] = frame_number
                    self.frame_sample_dict[scan_id] = sample_indices(frame_number, self.vggt_scan_sample_size)
                frame_index = self.frame_sample_dict[scan_id][frame_sample_index]
                N = self.frame_number_matrixcity[scan_id]
                half = self.vggt_valid_range // 2
                start = max(0, frame_index - half)
                end = start + self.vggt_valid_range
                if end > N:
                    end = N
                    start = max(0, end - self.vggt_valid_range)
                ranking_for_frame = list([frame_index]) + list(range(start, frame_index)) + list(range(frame_index+1, end))
                # c2w_list = []
                # for frame_id in ranking_for_frame:
                #     c2w, w2c = self.load_pose(scan_id, frame_id)
                #     c2w_list.append(c2w)
                # c2w_list = np.stack(c2w_list, axis=0)
                # translations = c2w_list[:, :3, 3]
                # dists = np.linalg.norm(translations - translations[0:1], axis=1)
                # ranking = np.argsort(dists)
                # ranking_for_frame = np.array(ranking_for_frame)[ranking]
                frame_tuple_index = sample_with_first(ranking_for_frame[:self.vggt_valid_range], self.num_images_in_tuple)
                frame_ids = frame_tuple_index
        else:
        ...
        if self.single_output_format:
            return self.stack_src_data(inputs)
        else:
...
def sample_indices(N, M):
    ...
    if M <= N:
        return random.sample(base, M)  # no replacement
    
    else:
        extra = random.choices(base, k=M - N)  # with replacement
        result = base + extra
        random.shuffle(result)
        return result
    

def sample_with_first(arr, M):
    arr = np.asarray(arr)

    if M == 0:
        return np.array([], dtype=arr.dtype)

    first = arr[:1]      # keep first element
    rest = arr[1:]

    replace = (M - 1) > len(rest)  # allow replacement if needed
    sampled = np.random.choice(rest, size=M - 1, replace=replace)

    return np.concatenate([first, sampled])
